...
INPUT_FILE = f"{SG_PATH}/story_elements_Glot150.txt"
RAW_OUTPUT_FILE = f"{SG_PATH}/llama_raw_output_Glot150.txt"
...
def run_llama_cli_batch(batch, batch_index):
    ...
    # print(f"Estimated tokens: {simple_token_estimate(prompt)}")
    ...
    try:
        ...
        # print("--- LLaMA Raw Output (partial) ---")
        # print(output[:2000] + ("\n..." if len(output) > 2000 else ""))
        # print("--- End of LLaMA Output ---\n")
...
def main():
    ...
    for i in range(0, len(story_elements), BATCH_SIZE):
        ...
        #print(f"Processing batch with {len(batch)} elements...")
        run_llama_cli_batch(batch, batch_index=(i // BATCH_SIZE) + 1)

    # print(f"\nRaw outputs saved to {RAW_OUTPUT_FILE}")
